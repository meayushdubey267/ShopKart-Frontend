...
export class CartService {
  ...
  // Subject is a subclass of Observable and can be used to publish events
  totalPrice: Subject<number> = new BehaviorSubject<number>(0);
  totalQuantity: Subject<number> = new BehaviorSubject<number>(0);

  // storage : Storage = sessionStorage; // use sessionStorage by default
  storage: Storage = localStorage; // use localStorage for persistence

  constructor() {
    // read data from session storage
    const data = JSON.parse(this.storage.getItem('cartItems')!);

    // if data is not null, assign it to cartItems
    if (data != null) {
      this.cartItems = data;
    }

    // compute the cart totals based on the data read from the storage
